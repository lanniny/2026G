...
proc ensure_port {name dir {from ""} {to ""} {type ""}} {
    set port [get_bd_ports -quiet $name]
    if {[llength $port] != 0} { return $port }
    set args [list -dir $dir]
    if {$from ne "" && $to ne ""} {
        lappend args -from $from -to $to
    }
    if {$type ne ""} {
        lappend args -type $type
    }
    lappend args $name
    return [create_bd_port {*}$args]
}
...
foreach connection {
    ...
    disconnect_intf_if_connected [lindex $connection 0]
    disconnect_intf_if_connected [lindex $connection 1]
}

foreach pin {
    ad9226test_0/SYS_CLK_IN
    ad9226test_0/SYS_RST_N_IN
    ad9226test_0/ADC_CLK_COMMON_OUT1
    ad9226test_0/ADC_CLK_COMMON_OUT2
    ad9226test_0/ADC_DATA_IN_PIN1
    ad9226test_0/ADC_DATA_IN_PIN2
    ad9226test_0/ps_capture_start
    ad9226test_0/ps_status
    ad9226test_0/m_axis_aclk
} {
    disconnect_pin_if_connected $pin
}

set old_adc [get_bd_cells -quiet ad9226test_0]
if {[llength $old_adc] != 0} {
    delete_bd_objs $old_adc
}
set adc [create_bd_cell -type module -reference ad9226test ad9226test_0]
...
ensure_port ADC_DATA_IN_PIN1_0 I 11 0
ensure_port ADC_DATA_IN_PIN2_0 I 11 0
ensure_port ADC_CLK_COMMON_OUT1_0 O
ensure_port ADC_CLK_COMMON_OUT2_0 O
...
foreach connection {
    ...
    connect_bd_intf_net [get_bd_intf_pins [lindex $connection 0]] [get_bd_intf_pins [lindex $connection 1]]
}
...
connect_bd_net [get_bd_ports ADC_DATA_IN_PIN1_0] [get_bd_pins ad9226test_0/ADC_DATA_IN_PIN1]
connect_bd_net [get_bd_ports ADC_DATA_IN_PIN2_0] [get_bd_pins ad9226test_0/ADC_DATA_IN_PIN2]
connect_bd_net [get_bd_pins ad9226test_0/ADC_CLK_COMMON_OUT1] [get_bd_ports ADC_CLK_COMMON_OUT1_0]
connect_bd_net [get_bd_pins ad9226test_0/ADC_CLK_COMMON_OUT2] [get_bd_ports ADC_CLK_COMMON_OUT2_0]
connect_bd_net [get_bd_pins axi_gpio_ad9226_ctrl/gpio_io_o] [get_bd_pins ad9226test_0/ps_capture_start]
connect_bd_net [get_bd_pins ad9226test_0/ps_status] [get_bd_pins axi_gpio_ad9226_ctrl/gpio2_io_i]

puts "AD9226_SYS_CLK_FREQ=[get_property CONFIG.FREQ_HZ [get_bd_pins ad9226test_0/SYS_CLK_IN]]"
puts "AD9226_AXIS_CLK_FREQ=[get_property CONFIG.FREQ_HZ [get_bd_pins ad9226test_0/m_axis_aclk]]"
