...
$temporaryOutput = Join-Path $bootRoot "BOOT.CDX.BIN"
...
if (Test-Path -LiteralPath $temporaryOutput) {
    throw "Temporary Bootgen output already exists; inspect it before retrying: $temporaryOutput"
}
...
try {
    $bootgenOutput = @(& $bootgen -arch zynq -image $bif -w on -o $temporaryOutput 2>&1)
    $bootgenExit = $LASTEXITCODE
} finally {
    Pop-Location
}
$bootgenOutput | ForEach-Object { Write-Output $_ }

if ($bootgenExit -ne 0) {
    throw "bootgen failed with exit code $bootgenExit"
}
if (-not ($bootgenOutput -match "Bootimage generated successfully")) {
    throw "bootgen exited without its success marker"
}
if (-not (Test-Path -LiteralPath $temporaryOutput)) {
    throw "bootgen returned success without creating $temporaryOutput"
}
$generatedInfo = Get-Item -LiteralPath $temporaryOutput
if ($generatedInfo.LastWriteTimeUtc -lt $buildStartedUtc.AddSeconds(-1)) {
    throw "bootgen output timestamp predates this build: $temporaryOutput"
}

$readOutput = @(& $bootgen -arch zynq -read $temporaryOutput 2>&1)
$readExit = $LASTEXITCODE
$readOutput | ForEach-Object { Write-Output $_ }
if ($readExit -ne 0) {
    throw "bootgen could not read back the generated image (exit $readExit)"
}

$previousOutput = $null
if (Test-Path -LiteralPath $output) {
    $previousOutput = Join-Path $bootRoot "BOOT.PREVIOUS.BIN"
    if (Test-Path -LiteralPath $previousOutput) {
        throw "Previous-image staging file already exists: $previousOutput"
    }
    Move-Item -LiteralPath $output -Destination $previousOutput
}
try {
    Move-Item -LiteralPath $temporaryOutput -Destination $output
} catch {
    if ($previousOutput -and
        (Test-Path -LiteralPath $previousOutput) -and
        -not (Test-Path -LiteralPath $output)) {
        Move-Item -LiteralPath $previousOutput -Destination $output
    }
    throw
}
if ($previousOutput -and (Test-Path -LiteralPath $previousOutput)) {
